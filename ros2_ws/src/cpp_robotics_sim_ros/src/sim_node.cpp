...
public:
  ...
  : Node("sim_node")
  {
    ...
    diagnostics_publisher_ =
      this->create_publisher<diagnostic_msgs::msg::DiagnosticArray>("/diagnostics", state_qos);
    ...
    RCLCPP_INFO(get_logger(),
      "QoS configured: /cmd_vel reliable volatile depth=10, "
      "/robot_pose reliable volatile depth=10, "
      "/odom reliable volatile depth=10");

    timer_ =
      this->create_wall_timer(std::chrono::duration_cast<std::chrono::milliseconds>(time_period),
...
private:
  bool validateParameters()
  {
    ...
    if (max_linear_velocity_ < 0.0) {
      RCLCPP_ERROR(
                get_logger(),
                "Invalid parameter: max_linear_velocity must be "
                ">= 0. Current max_linear_velocity =%.3f",
                max_linear_velocity_
      );
      valid = false;
    }
    if (max_angular_velocity_ < 0.0) {
      RCLCPP_ERROR(
                get_logger(),
                "Invalid parameter: max_angular_velocity must be "
                ">= 0. Current max_angular_velocity =%.3f",
                max_angular_velocity_
      );
      valid = false;
    }
  ...
  void cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
  {
    ...
    last_cmd_time_ = this->now();
    RCLCPP_INFO(get_logger(), "Received cmd_vel: linear.x=%.2f, angular.z=%.2f", linear_velocity_,
      angular_velocity_);
  }
  ...
  void timerCallback()
  {
    ...
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
      "Pose: x=%.2f, y=%.2f, theta=%.2f | v=%.2f, w=%.2f",
            pose_.x,
            pose_.y,
            pose_.theta,
            linear_velocity_,
  ...
  )
  {
    ...
    status.values.push_back(makeKeyValue("max_linear_velocity",
      std::to_string(max_linear_velocity_)));
    status.values.push_back(makeKeyValue("max_angular_velocity",
    ...
    status.values.push_back(makeKeyValue("average_callback_time_ms",
      std::to_string(average_callback_time_ms)));
    status.values.push_back(makeKeyValue("max_callback_time_ms",
